...
```html
<iframe id="pm-blog" src="https://pennmediated.github.io/blog/" title="Blog — Penn MEDIATED" scrolling="no" style="width:100%;height:3800px;border:0;display:block"></iframe><script>(function(){var f=document.getElementById('pm-blog');if(!f)return;var t=null,loaded=false;if(location.hash){f.src='https://pennmediated.github.io/blog/'+location.hash;}window.addEventListener('message',function(e){if(e.source!==f.contentWindow)return;var d=e.data||{},h=d.frameHeight||(d.type==='partners-page-resize'?d.height:0);if(h)f.style.height=h+'px';if(typeof d.frameScrollTo==='number'){t=d.frameScrollTo;go(loaded?'smooth':'auto');}});function go(b){if(t===null)return;window.scrollTo({top:f.getBoundingClientRect().top+window.pageYOffset+t-120,behavior:b});}window.addEventListener('load',function(){loaded=true;if(location.hash)setTimeout(function(){go('auto');},60);});})();</script>
```

The snippet does three things beyond sizing the frame. It passes the WordPress page's `#hash` into the frame, so links like `mediated.upenn.edu/blog/#knight-fellows` (the home page's What's New cards use these) open that post. It scrolls the WordPress page when the blog asks (`{ frameScrollTo }`): opening a post places the post box level with the post and brings it into view, instead of at the top of the frame. And it re-applies that scroll just after the WordPress page finishes loading, because Divi scrolls a page with a `#hash` back to the top at that point. `loading="lazy"` was dropped so a hashed link can open immediately.
